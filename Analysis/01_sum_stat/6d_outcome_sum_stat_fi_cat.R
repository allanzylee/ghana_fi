...
outcome_sum_stat_scale_func <- function(var_group_str, label_map) {
  ...
  data %>% 
    ...
    summarize(across(matches('per'), ~mean(., na.rm = TRUE), .names = "mean.{col}")) %>%
    mutate(
...
ggsave("/Users/AllanLee/Desktop/Personal Projects/ECON4900/Output/01_sum_stat/06d_outcome_sum_stat_fi_cat.png",
