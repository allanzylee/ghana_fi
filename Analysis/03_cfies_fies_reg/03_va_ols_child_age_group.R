...
va_ols_input_region <- expand.grid(category=c('lit','num','ef','sel'),
                                                 model=c('~ e_cfies_scale+e_fies_scale+e_cfies_scale*age+e_fies_scale*age+female+age+treatment+region_north_east+region_northern+region_upper_east+region_upper_west+age_pct_rank+factor(month)+'))
...
modelsummary(va_ols_region_results,
             ...
             fmt=f,
             cluster='careid',
             coef_omit = "^(?!.*tercept|.*indicator|.*outcome|.*age|.*[0-9])|.*treatment",
