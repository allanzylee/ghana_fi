...
# Author: Allan Lee
# Date: Nov 30th, 2023
# Purpose: Try IV results without combining FI and run each 2SLS model with the three versions of weather deviance IVs
...
full_data_w <- e_child %>% 
  ...
  dplyr::right_join(e_cg %>% select(careid, 
                                    childid, 
  ...
  by=c("childid",'careid')) %>% 
  dplyr::left_join(outcome %>% select(childid, 
                                      careid, 
                                      contains('per')),
                   by=c("childid","careid")) %>% 
  dplyr::left_join(cg_pe %>% select(childid,
                                    careid,
                                    pe_pc1=pc1,
                                    pe_pc2=pc2,
                                    pe_pc3=pc3,
                                    pe_pc4=pc4,),
                   by=c("childid","careid")) %>% 
  dplyr::left_join(treatment %>% select(-`...1`),
                   by=c("childid","careid")) %>%
  dplyr::left_join(child_order %>% select(childid,
                                          careid,
                                          poverty=mid_reverse_ppi,
                                          num_kids),
                   by=c("childid","careid")) %>%
  dplyr::left_join(iv %>% select(-`...1`),
                   by=c("careid","childid")) %>% 
  # dplyr::select(region.x,district.x,careid,childid,cr3.y,cr5_2,cr6,cr7,ed2,cb1,cb2,cb3,cb5,optimal_p_dev,optimal_t_dev,mean_p_dev,mean_t_dev,seasonal_p_dev,seasonal_t_dev,contains("extreme"),
  #        m_sel_per,m_lit_per,m_ef_per,m_num_per,e_sel_per,e_lit_per,e_ef_per,e_num_per,contains("fs_dummy"),PC1,PC2,PC3,PC4,mid_reverse_ppi,num_kids,io1,ed3,pe7,treatment,contains("gb")) %>% 
...
#   dplyr::select(e_ch_fs_dummy, e_cg_fs_dummy, female,age,enrolled_in_school,current_class,private_school,num_books,cg_age,cg_female,cg_edu,marital_status,poverty,num_kids, treatment) %>% 
...
#   select(childid,careid,contains("gb")) %>% 
#   dplyr::mutate_at(vars(gb1:gb15),~case_when(.==1~0,
#                                                T~1)) %>% 
#   select(-childid,-careid) %>% 
#   as.data.frame()
# 
# names(gender_bias)<-toupper(names(gender_bias))
# stargazer(gender_bias)

################################ Define relevant terms for regressions ###############################

variables<-c("Child-Reported Food Insecurity",
             "Caregiver-Reported Food Insecurity",
             "Midline Education Outcome",
             "Child Female",
             "Child Age",
             "Enrolled in School",
             "Class 1",
             ...
             "Poverty",
             "Household Size",
             "Parental Engagement PC1",
             "Parental Engagement PC2",
             "Parental Engagement PC3",
             "Parental Engagement PC4",
             "Participated in PNP Treatment Group",
             "Speaked Dagbani",
             "Speaked Gruni",
             "Speaked Other Language",
             "Speaked Sissali",
             "Constant")

variables_g<-c("Child-Reported Food Insecurity",
               "Caregiver-Reported Food Insecurity",
               "Child Female",
               "Midline Education Outcome",
               "Child Age",
               ...
               "Speaked Other Language",
               "Speaked Sissali",
               "Child-Reported FI: Child Female","Caregiver-Reported FI: Child Female","Constant")

variables_a<-c("Child-Reported Food Insecurity",
               "Caregiver-Reported Food Insecurity",
               "Child Age",
               "Midline Education Outcome",
               "Child Female",
               ...
               "Speaked Other Language",
               "Speaked Sissali",
               "Child-Reported FI: Child Age","Caregiver-Reported FI: Child Age","Constant")

iv_variables_cov_1l<-c("Mean Precipitation Deviance", "Seasonal Temperature Deviance",
             "Midline Education Outcome",
             "Child Female",
             "Child Age",
             "Enrolled in School",
             ...
             "Speaked Other Language",
             "Speaked Sissali",
             "Deviance Interaction Term",
             "Constant")

iv_variables_cov_1l_g <-c("Mean Precipitation Deviance", "Optimal Temperature Deviance",
                          "Midline Education Outcome", "Child Age",
  "Enrolled in School",
  "Class 1",
  "Class 2",
  "Class 3",
...
iv_variables_cov_2l_g <- replace(c("FI Indicator",variables_g[!variables_g %in% c('Parental Engagement PC2','Parental Engagement PC3','Parental Engagement PC4','Parental Engagement PC5', "Child-Reported Food Insecurity", "Caregiver-Reported Food Insecurity","Caregiver-Reported FI: Child Female")]),c(34),c("FI Indicator: Child Female"))

iv_variables_cov_1l_a <-c("Mean Temperature Deviance Indicator", "Optimal Temperature Deviance Indicator", "Optimal Temperature Deviance",
                          "Midline Education Outcome", "Child Female",
                          "Enrolled in School",
                          "Class 1",
                          "Class 2",
                          "Class 3",
...
iv_variables_cov_2l_a <- replace(replace(c("FI Indicator",variables_g[!variables_g %in% c('Parental Engagement PC2','Parental Engagement PC3','Parental Engagement PC4','Parental Engagement PC5', "Child-Reported Food Insecurity", "Caregiver-Reported Food Insecurity","Caregiver-Reported FI: Child Female")]),c(34),c("FI Indicator: Child Age")),c(2,4),c("Child Age", "Child Female"))

omitted_variables<-c('female','age','enrolled_in_school','current_class1','current_class2','current_class3','current_class4',
...
omitted_variables_g<-c('age','enrolled_in_school','current_class1','current_class2','current_class3','current_class4',
                       'current_class5','current_class6','current_class7','current_class8','current_class9','current_class10',
                       'current_class11','current_class12','current_class13','current_class14','private_school','num_books','cg_age','cg_female',
                       'marital_status','cg_edu','poverty','num_kids','pe_pc1',
                       'pe_pc2','pe_pc3','pe_pc4','treatment','languageDagbani','languageGruni',
...
omitted_variables_a<-c('female','enrolled_in_school','current_class1','current_class2','current_class3','current_class4',
                       'current_class5','current_class6','current_class7','current_class8','current_class9','current_class10',
                       'current_class11','current_class12','current_class13','current_class14','private_school','num_books','cg_age','cg_female',
                       'marital_status','cg_edu','poverty','num_kids','pe_pc1',
                       'pe_pc2','pe_pc3','pe_pc4','treatment','languageDagbani','languageGruni',
...
stargazer(first_stage_models_cov_frongillo,
          ...
          out="/Users/AllanLee/Desktop/Personal Projects/ECON4900/Output/11.30.2023_2sls_results_by_iv/first_stage_seasonal_iv.html")
...
stargazer(iv_cov_frongillo,
          ...
          out="/Users/AllanLee/Desktop/Personal Projects/ECON4900/Output/11.30.2023_2sls_results_by_iv/second_stage_seasonal_iv.html")
...
multivariate_first_stage_int_model_eq_ch <-paste(first_stage_int_ch,multivariate_iv_g_instruments,"+m_edu+age+enrolled_in_school +current_class+ private_school+num_books+cg_age +cg_female +marital_status+cg_edu +poverty+num_kids+pe_pc1+treatment+language")
...
multivariate_first_stage_int_g_model_eq_ch <-paste(first_stage_int_ch_g,multivariate_iv_g_instruments,"+m_edu+age+enrolled_in_school +current_class+ private_school+num_books+cg_age +cg_female +marital_status+cg_edu +poverty+num_kids+pe_pc1+treatment+language")
multivariate_first_stage_int_model_eq_cg <-paste(first_stage_int_cg,multivariate_iv_g_instruments,"+m_edu+age+enrolled_in_school +current_class+ private_school+num_books+cg_age +cg_female +marital_status+cg_edu +poverty+num_kids+pe_pc1+treatment+language")
multivariate_first_stage_int_g_model_eq_cg <-paste(first_stage_int_cg_g,multivariate_iv_g_instruments,"+m_edu+age+enrolled_in_school +current_class+ private_school+num_books+cg_age +cg_female +marital_status+cg_edu +poverty+num_kids+pe_pc1+treatment+language")
...
lit_child_fs_1l_multivariate_int_g_frongillo_ch<-lm(multivariate_first_stage_int_g_model_eq_ch,data=full_data_l %>% filter(m_outcome_type=="lit_per"))
summary(lit_child_fs_1l_multivariate_int_g_frongillo_ch)
lit_child_fs_1l_multivariate_int_g_frongillo_cg<-lm(multivariate_first_stage_int_g_model_eq_cg,data=full_data_l %>% filter(m_outcome_type=="lit_per"))
...
stargazer(first_stage_models_multivariate_int_g,
          ...
          out="/Users/AllanLee/Desktop/Personal Projects/ECON4900/Output/11.30.2023_2sls_results_by_iv/gender_first_stage_seasonal_iv.html")
...
stargazer(second_stage_models_multivariate_int_g,
          ...
          out="/Users/AllanLee/Desktop/Personal Projects/ECON4900/Output/11.30.2023_2sls_results_by_iv/gender_second_stage_seasonal_iv.html")
...
multivariate_iv_a_instruments <- "seasonal_p_dev + seasonal_t_dev+seasonal_p_dev*seasonal_t_dev+extreme_s_p_dev+extreme_s_t_dev"
multivariate_first_stage_int_model_eq_ch <-paste(first_stage_int_ch,multivariate_iv_a_instruments,"+m_edu+female+enrolled_in_school +current_class+ private_school+num_books+cg_age +cg_female +marital_status+cg_edu +poverty+num_kids+pe_pc1+treatment+language")
multivariate_first_stage_int_model_eq_cg <-paste(first_stage_int_cg,multivariate_iv_a_instruments,"+m_edu+female+enrolled_in_school +current_class+ private_school+num_books+cg_age +cg_female +marital_status+cg_edu +poverty+num_kids+pe_pc1+treatment+language")
...
multivariate_first_stage_int_a_model_eq_ch <-paste(first_stage_int_ch_a,multivariate_iv_a_instruments,"+m_edu+female+enrolled_in_school +current_class+ private_school+num_books+cg_age +cg_female +marital_status+cg_edu +poverty+num_kids+pe_pc1+treatment+language")
multivariate_first_stage_int_a_model_eq_cg <-paste(first_stage_int_cg_a,multivariate_iv_a_instruments,"+m_edu+female+enrolled_in_school +current_class+ private_school+num_books+cg_age +cg_female +marital_status+cg_edu +poverty+num_kids+pe_pc1+treatment+language")
...
stargazer(first_stage_models_multivariate_int_a,
          ...
          out="/Users/AllanLee/Desktop/Personal Projects/ECON4900/Output/11.30.2023_2sls_results_by_iv/age_first_stage_mean_iv.html")

####################################### Second Stage ###############################

########################################### Define the Equation ##############################

multivariate_iv_a_second_stage_model_eq <- paste("e_edu ~ e_ch_fs_dummy+e_cg_fs_dummy+age+e_ch_fs_dummy*age+e_cg_fs_dummy*age+m_edu+female+enrolled_in_school +current_class+ private_school+num_books+cg_age +cg_female +marital_status+cg_edu +poverty+num_kids+pe_pc1+treatment+language|",multivariate_iv_a_instruments,"+m_edu+female+enrolled_in_school +current_class+ private_school+num_books+cg_age +cg_female +marital_status+cg_edu +poverty+num_kids+pe_pc1+treatment+language")
...
stargazer(second_stage_models_multivariate_int_a,
          ...
          out="/Users/AllanLee/Desktop/Personal Projects/ECON4900/Output/11.30.2023_2sls_results_by_iv/age_second_stage_mean_iv.html")
